...
- 서버 측 response 저장
- 여러 응답을 생성하는 `n > 1`
- temperature, top_p, stop, seed, token limit 등의 sampling/generation 제어
...
LunaTranslator는 지원 클라이언트 중 하나일 뿐이며 프로젝트 전체가 Luna에 종속되지 않습니다. 대형 모델 범용 인터페이스에 같은 Base URL과 키를 입력하면 `/v1/chat/completions`를 사용할 수 있습니다.
