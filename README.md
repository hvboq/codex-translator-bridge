...
| system/developer 우선순위 | 지원 | App Server의 base/developer instructions로 전달 |
| 요청별 reasoning effort | 지원 | Chat `reasoning_effort`, Responses `reasoning.effort` |
...
- reasoning summary, mode, context 및 reasoning 내용 노출
- 임베딩, 이미지 생성, 오디오, 파인튜닝 등의 다른 OpenAI API

미지원 요청은 조용히 무시하지 않고 `400 invalid_request_error`로 반환합니다.

Chat의 `system` 메시지는 Codex App Server의 `baseInstructions`로, `developer` 메시지와 Responses의 최상위 `instructions`는 `developerInstructions`로 전달합니다. 이전 `user`·`assistant` 메시지만 검증된 대화 기록으로 주입하므로 시스템 프롬프트가 일반 과거 대화로 약화되지 않습니다. 여러 system/developer 메시지는 같은 역할 안에서 요청 순서대로 결합됩니다. 이 지시들은 응답 내용과 형식을 제어하지만 브리지의 도구 비활성화, 읽기 전용, 네트워크 차단 같은 보안 제약은 바꿀 수 없습니다.
...
Reasoning effort는 advisory가 아니라 실제 Codex turn 설정입니다. Chat은 `reasoning_effort`, Responses는 `reasoning: { "effort": "high" }`를 사용할 수 있습니다. 브리지는 선택 모델이 `GET /v1/models`에서 광고한 값을 요청 시작 전에 검증하고 `turn/start.effort`로 전달합니다. 실제 적용값은 `X-Codex-Bridge-Reasoning-Effort` 헤더와 Responses 결과의 `reasoning.effort`에서 확인할 수 있습니다. reasoning 과정 자체는 계속 외부로 내보내지 않습니다. `ultra`는 OpenAI 표준값이 아니라 Codex 모델 카탈로그가 광고하는 모델에서만 쓸 수 있는 확장값입니다.
...
Luna의 `사용자 정의 system prompt`는 Chat의 system 메시지로 받아 실제 App Server `baseInstructions`에 적용합니다. 설정 창에서 프롬프트 사용 스위치와 하단 `확인`을 누른 뒤 번역기를 다시 시작하면 별도의 user-message 반복 지시 없이도 대상 언어와 번역 규칙이 유지됩니다.

Luna의 `reasoning effort` 스위치와 `thinking.type`도 지원합니다. Luna 설정에 들어 있는 `xhign` 오타는 `xhigh`로 정규화합니다. `thinking.type=enabled`는 지정한 effort 또는 브리지 전역 기본값을 사용하고, `disabled`는 선택 모델이 완전 비활성화를 제공하지 않을 경우 지원되는 최저 단계로 내려갑니다. 전역 기본값의 `none`/`minimal`도 같은 방식으로 호환되지만, 요청에서 명시한 표준 effort가 선택 모델에서 지원되지 않으면 400을 반환합니다. fallback 시 `X-Codex-Bridge-Reasoning-Fallback` 헤더에 `none->low` 같은 실제 값을 표시합니다. `thinking.type=disabled`와 별도 effort가 함께 오면 disabled가 우선하고, `enabled`와 `none` 조합은 모순된 요청으로 거부합니다.
...
| `CODEX_BRIDGE_REASONING_EFFORT` | `low` | 기본 추론 강도; 요청별 값이 우선 |
