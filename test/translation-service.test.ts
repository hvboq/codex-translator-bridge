...
class FakeRunner implements StructuredRunner {
  ...
  async resolveModel(requested?: string): Promise<CodexModel> {
    const value = requested && !['codex-bridge', 'codex-translator'].includes(requested)
      ? requested
      : 'gpt-5.6-sol';
  ...
  async runStructured(
    ...
    throw new Error('Unexpected structured prompt');
  }

  async runText(): Promise<never> {
    throw new Error('Translation tests must not use general text generation');
  }
}

test('micro-batches misses and persists cache hits', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'codex-bridge-translation-test-'));
...
test('separates cache, in-flight work, and micro-batches by resolved model', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'codex-bridge-model-test-'));
  const config = testConfig(directory);
  const cache = new TranslationCache(config.cacheFile, 100);
  await cache.initialize();
  const runner = new FakeRunner();
...
test('protects and restores game formatting tokens', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'codex-bridge-token-test-'));
  const config = testConfig(directory);
  const cache = new TranslationCache(config.cacheFile, 100);
  await cache.initialize();
  const runner = new FakeRunner();
...
test('rejects oversized and malformed translation inputs', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'codex-bridge-limit-test-'));
...
test('recovers the persistent cache queue after a write failure', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'codex-bridge-cache-test-'));
